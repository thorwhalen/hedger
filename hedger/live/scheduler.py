...
def run_scheduler(
    ...
    *,
    ...
    interval = {"1m": 1, "5m": 5, "15m": 15, "1h": 60, "1d": 60 * 24}.get(
        cfg.timeframe, 60
    )
    sched.add_job(
        runner.tick, IntervalTrigger(minutes=interval), name="tick", coalesce=True,
        max_instances=1,
    )
    if cfg.reflection.enabled and on_reflect:
        sched.add_job(
            on_reflect, CronTrigger.from_crontab(cfg.reflection.cron),
            name="reflect", max_instances=1,
        )
    log.info("scheduler_starting", cadence_minutes=interval,
             reflect_cron=cfg.reflection.cron)
    sched.start()
